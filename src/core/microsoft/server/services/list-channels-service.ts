...
import type { MicrosoftChannels } from "@/core/microsoft/domain/types";
...
export async function listMicrosoftChannelsService(
    organizationId: string,
    userId: string,
    teamId: string,
): AsyncAppResult<MicrosoftChannels> {
    ...
    try {
        ...
        const items = await listChannels(token.data, teamId);
        return ok({ items });
    } catch (cause) {
